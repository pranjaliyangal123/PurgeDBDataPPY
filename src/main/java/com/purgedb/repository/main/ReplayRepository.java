...
import com.purgedb.entity.Replay;
...
/**
 * Repository for REPLAY table (Main Database)
 */
@Repository
public interface ReplayRepository extends JpaRepository<Replay, Long> {
     ...
     */
    @Query("SELECT r FROM Replay r WHERE r.createdDate < :cutoffDate")
    List<Replay> findOldRecords(@Param("cutoffDate") LocalDateTime cutoffDate);
     ...
     */
    @Query("SELECT COUNT(r) FROM Replay r WHERE r.createdDate < :cutoffDate")
